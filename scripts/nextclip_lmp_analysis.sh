...
nextclip=/Users/leggettr/Downloads/nextclip-master/bin/nextclip
...
function run_alignment
{
    ...
    do
        ...
        do
            ...
            saifile=${bwadir}/${lib}_${type}_R${read}.sai
            samfile=${bwadir}/${lib}_${type}_R${read}.sam
...
function parse_alignment
{
    for type in A B C D
    do
        sam_one=${bwadir}/${lib}_${type}_R1.sam
        sam_two=${bwadir}/${lib}_${type}_R2.sam
        out_prefix=${analysisdir}/${lib}_${type}
...
{
    command="${scriptdir}/nextclip_plot_graphs.sh ${libdir} ${lib}"
...
{
    command="${scriptdir}/nextclip_make_report.sh -d ${libdir} -l ${lib} -r \"${reference}\" -o \"${organism}\""
